$username = 'dev-ad\admintcm'
$password = '7tdk43cd#01'
$ovc = "192.168.1.33"
...
$vmid = 'cdb3022d-2e8e-4f7a-b327-e9adef24efd0'
...
write-host $vm.name
write-host "$($vmid).State=$state"
write-host "$($vmid).sent=$sent"
write-host "$($vmid).name=$name"
write-host "$($vmid).created_at=$created_at"
write-host "$($vmid).size=$size"
write-host "$($vmid).application_consistent=$consistent"
